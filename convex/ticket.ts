...
// One-shot prod cleanup for the Nima duplicate-link incident discovered
// 2026-05-23. Two accounts share lumaGuestId gst-HJ5gwHAyDB11YKv:
//   - k57bd32qk28kmpjcmw5t3akg6s879w4b nima@techeurope.io (admin, first claim)
//   - k57d13n9xkr44f5fvbzw12gr8s878f2j nima@netbird.io (second claim via email_code)
// Plan: report any vouchers on the netbird account, move unredeemed ones
// to the techeurope account, delete the netbird ticketLinks row, clear
// the netbird user's ticket fields. Audit. Idempotent if re-run.
// REMOVE after running once on prod.
export const _diagCleanupNimaDuplicateLink = internalMutation({
  args: { dryRun: v.boolean() },
  handler: async (ctx, { dryRun }) => {
    const TECHEUROPE_USER_ID = "k57bd32qk28kmpjcmw5t3akg6s879w4b" as Id<"users">;
    const NETBIRD_USER_ID = "k57d13n9xkr44f5fvbzw12gr8s878f2j" as Id<"users">;
    const LUMA_GUEST_ID = "gst-HJ5gwHAyDB11YKv";

    const techeurope = await ctx.db.get(TECHEUROPE_USER_ID);
    const netbird = await ctx.db.get(NETBIRD_USER_ID);
    if (!techeurope || !netbird) {
      return { error: "expected users not found", techeurope, netbird };
    }

    const links = await ctx.db
      .query("ticketLinks")
      .withIndex("by_luma_guest_id", (q) =>
        q.eq("lumaGuestId", LUMA_GUEST_ID),
      )
      .collect();
    const netbirdLink = links.find((l) => l.userId === NETBIRD_USER_ID);
    const techeuropeLink = links.find((l) => l.userId === TECHEUROPE_USER_ID);

    const netbirdVouchers = await ctx.db
      .query("vouchers")
      .withIndex("by_user", (q) => q.eq("userId", NETBIRD_USER_ID))
      .collect();
    const unredeemedNetbird = netbirdVouchers.filter((v) => !v.redeemedAt);
    const redeemedNetbird = netbirdVouchers.filter((v) => v.redeemedAt);

    const plan = {
      linksFound: links.length,
      hasNetbirdLink: !!netbirdLink,
      hasTecheuropeLink: !!techeuropeLink,
      netbirdVouchers: netbirdVouchers.length,
      unredeemedToMove: unredeemedNetbird.length,
      redeemedStaying: redeemedNetbird.length,
      vouchersDetail: netbirdVouchers.map((v) => ({
        kind: v.kind,
    ...
    // Move unredeemed vouchers from netbird → techeurope.
    const movedVoucherIds: string[] = [];
    for (const vch of unredeemedNetbird) {
      await ctx.db.patch(vch._id, {
        userId: TECHEUROPE_USER_ID,
        email: techeurope.email,
      });
      movedVoucherIds.push(vch._id);
    }

    // Delete the netbird ticketLinks row.
    if (netbirdLink) await ctx.db.delete(netbirdLink._id);

    // Clear ticket fields on the netbird user.
    await ctx.db.patch(NETBIRD_USER_ID, {
    ...
    await ctx.db.insert("auditLog", {
      actorUserId: TECHEUROPE_USER_ID,
      action: "ticket.duplicate_link_cleanup",
      targetUserId: NETBIRD_USER_ID,
      metadata: JSON.stringify({
        lumaGuestId: LUMA_GUEST_ID,
        deletedLinkId: netbirdLink?._id ?? null,
        movedVouchers: movedVoucherIds,
        redeemedStaying: redeemedNetbird.map((v) => v._id),
        note: "one-shot cleanup, see CLAUDE Code session 2026-05-25",
      }),
      createdAt: now,
    });

    return { applied: true, plan, movedVoucherIds };
  },
});
