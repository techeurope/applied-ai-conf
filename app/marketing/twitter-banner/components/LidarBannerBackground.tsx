...
interface LidarBannerBackgroundProps {
  fogDensity?: number;
  gridOpacity?: number;
...
export function LidarBannerBackground({
  fogDensity = 2,
  gridOpacity = 0.55,
  animationPaused = false,
  animationTime = 0,
}: LidarBannerBackgroundProps) {
  ...
  const uniforms = useRef({
    ...
    uColor: { value: new THREE.Color("#ab7030") }, // Amber color from landing page
  ...
  useEffect(() => {
    uniforms.current.uFogDensity.value = fogDensity;
    uniforms.current.uOpacity.value = gridOpacity;
  }, [fogDensity, gridOpacity]);
  ...
  useFrame((state) => {
    if (meshRef.current) {
      ...
      // Update fog and opacity
      material.uniforms.uFogDensity.value = fogDensity;
      material.uniforms.uOpacity.value = gridOpacity;
    }
  });
  ...
  const fragmentShader = `
    ...
    void main() {
      ...
      // Fog based on distance with adjustable density
      float fogFactor = smoothstep(0.0, 1.0 / uFogDensity, vDistance * 0.05);
      float opacity = (1.0 - fogFactor) * lineStrength;
      
      // Vignette for top/bottom fade
      float vignette = smoothstep(0.0, 0.15, vUv.y) * smoothstep(1.0, 0.85, vUv.y);
      opacity *= vignette;
